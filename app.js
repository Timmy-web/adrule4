...
function list_files(path,files){
    ...
    for(i in files){
        ...
        if(item['mimeType'] == 'application/vnd.google-apps.folder'){
            ...
            var ext = p.split('.').pop();
            if("|html|php|css|go|java|js|json|txt|sh|md|mp4|webm|avi|bmp|jpg|jpeg|png|gif|m4a|mp3|wav|ogg|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|flac|".indexOf(`|${ext.toLowerCase()}|`) >= 0){
	            p += "?a=view";
	            c += " view";
            }
...
function file_video(path){
	...
	if (/(iPhone|iPad|iPod|iOS|Android)/i.test(navigator.userAgent)) { //移动端
	    playBtn = `<a class="mdui-btn mdui-btn-raised mdui-ripple mdui-color-theme-accent" href="intent:${url}#Intent;package=com.newin.nplayer.pro;S.title=${path};end"><i class="mdui-icon material-icons">&#xe039;</i>在nPlayer中播放</a>`;
	}
	var content = `
